...
export default class CurrencyFormat {
  ...
  constructor(options: CurrencyFormatOptions) {
    ...
    if (this.decimalSymbol === undefined) {
      ...
      this.minimumFractionDigits = precision?.min ?? numberFormat.resolvedOptions().minimumFractionDigits ?? 0
      this.maximumFractionDigits = precision?.max ?? numberFormat.resolvedOptions().maximumFractionDigits ?? 0
    }
