...
export function useCurrencyInput({
  ...
  if (isRef(options)) {
    watch(options, (newOptions: CurrencyInputOptions) => {
      currencyInput?.setOptions(newOptions)
    })
  }

  if (modelValue !== undefined) {
    watch(modelValue, (value: number | string | null) => {
  ...
  watch(inputRef, (value: HTMLInputElement | ComponentPublicInstance | null) => {
    if (value) {
