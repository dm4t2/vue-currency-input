...
describe('CurrencyFormat', () => {
  ...
  describe('parse', () => {
    ...
    it('should return the parsed number if the value conforms to the currency format', () => {
      ...
      expect(new CurrencyFormat({ locale: 'ar-SA', currency: 'SAR' }).parse('١٢٣٤')).toBe(123400n)
      expect(new CurrencyFormat({ locale: 'ar-SA', currency: 'SAR' }).parse('١٬٢٣٤')).toBe(123400n)
      expect(new CurrencyFormat({ locale: 'ar-SA', currency: 'SAR' }).parse('٠٫٩')).toBe(90n)
      expect(new CurrencyFormat({ locale: 'ar-SA', currency: 'SAR' }).parse('-‏٠٫٥٠ ر.س.')).toBe(-50n)
